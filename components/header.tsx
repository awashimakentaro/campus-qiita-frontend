...
export function Header() {
  ...
  // --- ここがハイドレーション対策 ---
  // マウント前は必ず同じDOM（プレースホルダ）を描画してSSRと一致させる
  const Actions = () => {
    if (!mounted) {
      return (
    ...
    // マウント後にだけ、認証状態で分岐
    const loginUrl = `${process.env.NEXT_PUBLIC_API_BASE}/auth/login?redirect=${
      ...
      <div className="flex items-center space-x-4">
        ...
        {user ? (
          ...
          <Button asChild variant="outline">
            {/* フロントの /login ではなく BE の /auth/login に直接飛ばすと確実 */}
